...
var initTocHide=()=>{var onScreenItems=[];let hideTocCallback=(entries,observer)=>{entries.forEach((entry)=>{if(entry.isIntersecting){onScreenItems.push(entry.target);}else{for(let ii=0;ii<onScreenItems.length;ii++){if(onScreenItems[ii]===entry.target){onScreenItems.splice(ii,1);break}}};});if(onScreenItems.length>0){$("div.bd-toc").removeClass("show")}else{$("div.bd-toc").addClass("show")}};let manageScrolledClassOnBody=(entries,observer)=>{if(entries[0].boundingClientRect.y<-20){document.body.classList.add("scrolled");}else{document.body.classList.remove("scrolled");}}
